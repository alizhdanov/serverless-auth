...
describe('login', () => {
    it('returns error if empty body', async () => {
        ...
        expect(noBody.body).toBe('{"errors":["Please provide email.","Please provide password."]}');
    ...
    it('returns error if body or email not provided', async () => {
        ...
        const data = await handler({body: '{"email": "123@123.com", "password": "1234"}'});
        expect(data.statusCode).toBe(400);
    ...
    it('returns token on success', async () => {
        ...
        const data = await handler({body: '{"email": "123@123.com", "password": "123"}'});
